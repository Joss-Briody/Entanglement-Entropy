...
void Contractions_init( MatrixXd& A_A, MatrixXd& B_B, MatrixXd& A_B, 
				   MatrixXd* phi_mat_p, MatrixXd* psi_mat_p, 
					...
					double t, const int L)  { 											//need to check that using correct eigenvalues/vectors
	...
	for( int i = 0; i < L; i++ ) {
		for(int j = 0; j < L; j++ ) {
			...
			for(int q = 0; q < L; q++) {
				...
				x += sin( ( pow(((*eigenval_v_p)(q)),0.5) )*t)*( (*phi_mat_p).col(q)(i) )*( (*psi_mat_p).col(q)(j) );  //eigenvalues all positive (as spectrum bounded)
				y += cos( ( pow(((*eigenval_v_p)(q)),0.5) )*t)*( (*psi_mat_p).col(q)(i) )*( (*psi_mat_p).col(q)(j) );
				z += cos( ( pow(((*eigenval_v_p)(q)),0.5) )*t)*( (*phi_mat_p).col(q)(i) )*( (*phi_mat_p).col(q)(j) );
...
void Correlation_mat_create( MatrixXd* A_A, MatrixXd* B_B, MatrixXd* A_B, MatrixXd* static_correlation, MatrixXd& correlation_mat, const int L ) {
	...
	for(int l=1; l <= L; l++) {
		for(int m = 1; m <= L; m++) {
			...
			correlation_mat(2*l-1,2*m-1) =  ( prod_4(m-1,l-1) - prod_4(l-1,m-1) )*pow(-1, (l+m-2));    //NB changed factors of -1
		}
	}
}
...
int main() {
	...
	const int L      = 987;//1597;//20;//987;//377;//377; //144; //377;  // L = F_14
	const int sub_L  = 377;//610; //10;//377;//233;	
	const double h   = 0.50;
	const double h_0 = 0;//1000000000000;
	const double r   = 0.25; 						 // r = 1 -> homogeneous chain.  
	//const double t   = 600.0;
	const long double omega = 0.5L*(sqrt(5L) + 1L);
	const double J   = pow(r,-(1-(1/omega)));
	const int N_SIGN = -1;//pow((-1),L); 				//exp(i*pi*N) factor in Jordan-Wigner.
	...
	for(int i = 0; i < L; i++) {               //Hamiltonian before the quench.
		for(int j = 0; j < L; j++){
			...
			B_0(j,i) = -B_0(i,j);  					 //B_0 is antisymmetric	
		}
	}
...
*/
	...
	double dt 	 = 1.04;
	ofstream ENTROPY;
	ENTROPY.open("Fibonacci_no_bug_qu_small.txt");//"Fibonacci_test_F17.txt");
	for( double time = 0.05; time < T_end; time *= dt ) {
		...
		for(int i = 2*233; i < 2*sub_L + 2*233; i++) {
				for(int j = 2*233; j < 2*sub_L + 2*233; j++) {
					reduced_correlation_mat(i-(2*233),j-(2*233)) = correlation_mat(i,j);
				}
		}	
