...
//for(unsigned int i = 0; i< Output[4].size(); i++ )
//	cout<<Output[4][i]<<endl;
...
// Have all the data stored in Output, one vector for each time-step. Now need to correctly index this data:
 ...
 //vector< vector<double> > correlation_mat;
	...
	int t_band = static_cast<int>(18/dt);
	//vector< vector< vector<double> > > ridic;
	//vector<double> entropy_v;
...
//vector<vector<int>> A(dimension, vector<int>(dimension));
		...
		//correlation_mat.push_back(correlation_mat_t);              //Correlation_mat_t is a single time-step with data stored as [-f_0,g_0,-g_0,f_0;-f_-1,...]
	//} 																				 //Correlation_mat is a vector where each entry is the correlation matrix at a given time-step.
...
MatrixXd mat = MatrixXd::Constant(SIZE, SIZE, 0);
	//cout<<mat<<endl;

  //ofstream test;
  //test.open ("testDATA.txt");

//for(unsigned int q = 0; q < correlation_mat_t.size(); q++) {
	//test << "\n";

//cout << correlation_mat_t[q].size() << "\n";

//}
//test.close();

//cout<<my_data[0].size()<<endl;  //Need to add row by row for vector of vectors where memory not continguous
...
vector<double> eigenvalues_t (2*L,0);  //_t
for(unsigned int i = 0; i < 2*L; i++ )
	eigenvalues_t[i] = (es.eigenvalues().imag()[i]);

///////////

	//vector<double> entropy (eigenvalues.size(),0);
	
//double entropy_t = ( Entropy(&eigenvalues_t) );
//cout<<"entropy is: " << entropy_t << endl;
//entropy_t = 0;
//}

//for(unsigned int y = 0; y < entropy_v.size(); y++ ) {
//	cout << entropy_v[y] << endl;
//}
...
//testintegral << setprecision(10) << t << "\t" << l << "\t" << f_U  << "\t" << g_U << "\n" ;
	   

//test.close();
//cout << "The eigenvalues of A are:" << endl << es.eigenvalues().imag()[1] << endl;
}
//declare more const global variables, clean up, test quench values dont kill it
