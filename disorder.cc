...
vector<double> random_uniform( unsigned long int seed = 0, unsigned int size = 20 )
{
                  ...
                  double x1, double& error, int l, double t, double h_rand )
{
   ...
   double result;
   gsl_function F;
   struct quench_params Quench = {l, h_rand, 0, t};		//set the parameters for the quench. NB choosing either h or h_0 == 1 gives a zero in E or E_0 for \phi in [-\pi, \pi], but can choose 0.99999.
   F.function = func;
   F.params = &Quench;
 
   // Use a GSL routine for integrals:

   gsl_integration_qag( &F, x0, x1, 0.8e-14, 1e-7, max_intervals, GSL_INTEG_GAUSS61,    //qags in the gsl routine if function has integrable singularity. Then don't require GSL_INTEG_GAUSS61 argument.
